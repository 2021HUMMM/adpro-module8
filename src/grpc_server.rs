...
impl ChatService for MyChatService {
    ...
    async fn chat(
        ...
        tokio::spawn(async move {
            while let Some(msg) = stream.message().await.unwrap_or_else(|_| None) {
                println!("Received message: {:?}", msg);
                let reply = ChatMessage {
                    user_id: msg.user_id.clone(),
                    message: format!("Terima kasih telah melakukan chat kepada CS virtual. Pesan anda akan dibalas pada jam kerja. Pesan anda : {}", msg.message),
                };
